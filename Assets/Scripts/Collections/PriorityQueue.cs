...
// 移植自 .NET 6.0+ 中的 System.Collections.Generic.PriorityQueue<TElement, TPriority> 实现，做了部分删减和改动以适配 Unity 环境和满足特定需求。
...
namespace Assets.Collections
{
    ...
    public class PriorityQueue<TElement, TPriority>
    {
        ...
        public const int MaxArrayLength = 0x7FFFFFC7;  // 即 2147483591
        ...
        public sealed class UnorderedItemsCollection : IReadOnlyCollection<(TElement Element, TPriority Priority)>, ICollection
        {
            ...
            void ICollection.CopyTo(Array array, int index)
            {
                ...
                if (array.Rank != 1)
                {
                    throw new ArgumentException("Multi-dimensional array is not supported.", nameof(array));
                }

                if (array.GetLowerBound(0) != 0)
                {
                    throw new ArgumentException("Array with non-zero lower bound is not supported.", nameof(array));
                }

                if (index < 0 || index > array.Length)
                {
                    throw new ArgumentOutOfRangeException(nameof(index), index, "Index must be less or equal to array length.");
                }
            ...
            IEnumerator<(TElement Element, TPriority Priority)> IEnumerable<(TElement Element, TPriority Priority)>.GetEnumerator()
            {
                return _queue.Count == 0
                    ? Enumerable.Empty<(TElement Element, TPriority Priority)>().GetEnumerator()
                    : GetEnumerator();
            }
